		...
		for (int i = 0; i < 3; i++) {
			if (main.playerList[i].size() == 0) {
				...
				if (i == 1) {
					...
					money= user.getMoney()+500;
					user.setMoney(money);
					String sql = "UPDATE user SET money='"+user.getMoney()+"' WHERE username='"+user.getName()+"'";
					String sql2 ="insert into his_ach values('赢','"+sqlDate+"',"+user.getName()+")";
					
					int j=Dao.executeUpdate(sql);
					int q=Dao.executeUpdate(sql2);
					
				} else {
					s = "恭喜电脑" + i + ",赢了! 你的智商有待提高哦。金币-500";
					int money;
					java.util.Date date = new java.util.Date();
					java.sql.Date sqlDate=new java.sql.Date(date.getTime());
					money= user.getMoney()-500;
					user.setMoney(money);
					String sql = "UPDATE user SET money='"+user.getMoney()+"' WHERE username='"+user.getName()+"'";
					String sql2 ="insert into his_ach values('输','"+sqlDate+"',"+user.getName()+")";
					int j=Dao.executeUpdate(sql);
					int q=Dao.executeUpdate(sql2);
