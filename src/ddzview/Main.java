...
import com.ddz.model.Card;
import com.ddz.model.CardType;
import com.ddz.model.Common;
import com.ddz.model.Time;
import com.ddz.model.User;

public class Main extends JFrame implements ActionListener,Runnable{

	public static User user;//用户
	public Container container = null;// 定义容器
	JMenuItem start, exit, about;// 定义菜单按钮
	public JButton landlord[]=new JButton[2];//抢地主按钮
	public JButton publishCard[]=new JButton[2];//出牌按钮
	public int dizhuFlag;//地主标志
	public int turn;
	public JLabel dizhu; //地主图标
	...
	Card card[] = new Card[56]; // 定义56张牌
	public JTextField time[]=new JTextField[3]; //计时器
	Time t; //定时器（线程）
	...
	public void CardInit() {
		...
		//打乱顺序
		/*
		 * for(int i=0;i<100;i++){ Random random=new Random(); int
		 * a=random.nextInt(52)+1; int b=random.nextInt(52)+1; Card k=card[a];
		 * card[a]=card[b]; card[b]=k; }
		 */
		//开始发牌
		for(int i=0;i<3;i++)
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == about) {
			JOptionPane.showMessageDialog(this, "2018401178");
		}
	...
	public static void main(String args[]) {
			...
			new Thread(new Main(user)).start();
